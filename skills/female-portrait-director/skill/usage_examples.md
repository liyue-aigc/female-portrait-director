...
下面是部分内置风格的简短调用入口。V1.5 会通过风格注册表按需加载单一路由，锁定参数或授权参考图主体，完整扩写年龄、五官、身形、服装、姿态、场景、镜头、光线和滤镜，再将这些模块融合为摄影导演式提示词或直接生成图片。当前已实现 19 条 Route，完整列表见 [style-registry.md](style-registry.md)，首次使用教程和完整参数转提示词示例见 [help.md](help.md)，详细旧版示例位于 `examples/`。

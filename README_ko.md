...
여성 인물 프롬프트 디렉터 Skill은 AI 이미지 생성을 위한 구조화된 프롬프트 생성 및 시각 디렉션 시스템입니다. V1.5는 구현된 19개 라우트 중 필요한 하나만 불러오고, 명시된 파라미터 또는 승인된 참조 이미지의 주체를 고정하여 완전한 프롬프트나 주체 보존형 이미지 편집을 생성합니다.
...
파라미터 없이 처음 호출하면 19개 스타일 목록, 입력 템플릿, 파라미터에서 상세 프롬프트로 변환되는 예시, 직접 이미지 생성 및 승인된 참조 이미지 사용법이 포함된 V1.5 튜토리얼이 표시됩니다.
...
- 작성자: Li Yue (李岳)
- 버전: `FEMALE-PORTRAIT-DIRECTOR-V1.5`
